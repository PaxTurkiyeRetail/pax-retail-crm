...
type ViewTab = 'projects' | 'team' | 'budget';
...
const EMPTY_SUMMARY: Summary = { activeProjects: 0, answered: 0, pending: 0, open: 0, overdue: 0, noBlocker: 0, resolved: 0, riskQuantity: 0, completionRate: 0 };
...
function riskText(row: Row) {
  if (row.impact_type !== 'month_shift' || !row.shifted_quantity) return 'Satış kayması beklenmiyor';
  return `${row.shift_period_label || periodLabel(row.shift_year, row.shift_month)} dönemine ${numberFormat(row.shifted_quantity)} adet`;
}
...
export default function BlockerImpactClient() {
  ...
  const [view, setView] = useState<ViewTab>('projects');
  ...
  useEffect(() => {
    if (!isAdmin && view !== 'projects') setView('projects');
  }, [isAdmin, view]);

  const statusTabs = useMemo(() => [
    { value: '', label: 'Tümü', count: summary.activeProjects },
  ...
  function openForm(row: Row) {
    ...
    setForm({
      ...
      shiftYear: String(row.shift_year ?? Math.max(row.forecast_year, YEARS[0] ?? row.forecast_year)),
    ...
    try {
      ...
      if (isAdmin) {
        await readJson(await fetch('/api/forecast/blockers/review', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ forecast_id: selected.forecast_id, manager_note: form.managerNote, reviewed: form.reviewed }),
        }));
      }

      appToast.success('Kayıt güncellendi', `${selected.musteri} projesinin Engel ve Etki bilgisi kaydedildi.`);
      setSelected(null);
      setForm(emptyForm());
      await loadData();
    } catch (error: any) {
  ...
  async function updateWorkflow(row: Row, action: 'resolve' | 'reopen') {
    const question = action === 'resolve'
      ? `${row.musteri} projesindeki engel çözüldü olarak işaretlensin mi?`
      : `${row.musteri} projesindeki engel yeniden açılsın mı?`;
    if (!window.confirm(question)) return;
    try {
      await readJson(await fetch(`/api/forecast/blockers/${action}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ forecast_id: row.forecast_id }),
      }));
  ...
  async function exportWorkbook() {
    ...
    try {
      ...
      const blob = await buildBlockerImpactWorkbook([
        {
          ...
          widths: [20, 26, 18, 22, 15, 42, 24, 16, 18, 14, 20, 18],
          rows: [
            ['Account', 'Müşteri', 'Ürün Kodu', 'Ürün', 'Mevcut Dönem', 'Satışın Önündeki Engel', 'Kim Çözecek?', 'Çözüm Tarihi', 'Kayacağı Dönem', 'Kayacak Adet', 'Durum', 'Son Güncelleme'],
            ...exportRows.map((row) => [
              row.sorumlu ?? '-',
              row.musteri,
              row.product_code_snapshot ?? '-',
              row.product_name_snapshot,
              row.forecast_period_label,
        ...
        },
        {
          ...
          widths: [24, 15, 15, 15, 15, 15, 18],
          rows: [
            ['Kullanıcı', 'Aktif Proje', 'Cevaplanan', 'Cevap Bekleyen', 'Açık Engel', 'Tarihi Geçen', 'Tamamlama Oranı'],
            ...(json.completionByOwner ?? []).map((item) => [item.owner, item.activeProjects, item.answered, item.pending, item.open, item.overdue, `%${item.completionRate}`]),
          ],
        },
        {
      ...
      appToast.success('Excel hazırlandı', 'Engel ve Etki listesi üç sayfa olarak indirildi.');
    } catch (error: any) {
  ...
  const kpis = [
    { label: 'Aktif Proje', value: summary.activeProjects, hint: isAdmin ? 'Tüm ekip Forecast satırları' : 'Sorumluluğunuzdaki Forecast satırları', icon: ClipboardCheck, tone: 'slate' },
    { label: 'Cevap Bekleyen', value: summary.pending, hint: summary.pending ? 'Bilgi girişi gerekiyor' : 'Tüm projeler cevaplandı', icon: CircleAlert, tone: summary.pending ? 'amber' : 'green' },
    { label: 'Açık Engel', value: summary.open, hint: 'Takip edilmesi gereken proje', icon: AlertTriangle, tone: summary.open ? 'red' : 'green' },
    ...
    <main className="blocker-page premium-workspace">
      <section className="blocker-hero">
        <div className="blocker-hero-copy">
          ...
          <p>Aktif projelerde satışın önündeki engeli, çözüm sorumlusunu ve gecikmenin satış bütçesine etkisini tek yerde takip edin.</p>
        ...
        <div className="blocker-hero-actions">
          {isAdmin ? <button type="button" className="blocker-btn light" onClick={() => void exportWorkbook()} disabled={exporting}><Download size={17} /> {exporting ? 'Hazırlanıyor...' : 'Excel İndir'}</button> : null}
      ...
      {onboarding ? (
        <section className="blocker-setup-alert">
          <AlertTriangle size={22} />
          <div><strong>Engel ve Etki SQL kurulumu bekleniyor.</strong><span><code>sql/forecast_blocker_impact_setup.sql</code> dosyasını PostgreSQL üzerinde çalıştırın.</span></div>
        </section>
      ) : null}
      ...
      {summary.pending > 0 && !onboarding ? (
        <section className="blocker-action-banner">
          ...
          <div>
            <strong>{isAdmin ? `${summary.pending} proje için ekip cevabı bekleniyor` : `${summary.pending} projeniz için kısa değerlendirme bekleniyor`}</strong>
            <span>{isAdmin ? 'Kullanıcı Tamamlama sekmesinden eksik cevapları kişi bazında takip edebilirsiniz.' : 'Her proje için yalnızca üç soruyu cevaplamanız yeterli.'}</span>
          </div>
          <button type="button" onClick={() => { setView('projects'); setStatus('pending'); setPage(1); }}>Bekleyenleri Aç <ArrowRight size={16} /></button>
        </section>
      ) : null}
      ...
      <section className="blocker-kpi-grid">
        {kpis.map((item) => {
          ...
          return (
            <article key={item.label} className={`blocker-kpi tone-${item.tone}`}>
              <span className="blocker-kpi-icon"><Icon size={19} /></span>
              <div><span>{item.label}</span><strong>{numberFormat(item.value)}</strong><small>{item.hint}</small></div>
            </article>
          );
        })}
      </section>
      ...
      {isAdmin ? (
        <nav className="blocker-view-tabs" aria-label="Yönetim görünümleri">
          <button type="button" className={view === 'projects' ? 'active' : ''} onClick={() => setView('projects')}><ClipboardCheck size={18} /><span>Proje Listesi</span></button>
      ...
      {view === 'projects' ? (
        <>
          <section className="blocker-filter-card">
            <div className="blocker-filter-top">
              <div><span>Proje Listesi</span><h2>{isAdmin ? 'Tüm ekipteki engelleri yönetin' : 'Sorumluluğunuzdaki projeleri tamamlayın'}</h2></div>
              <button type="button" className="blocker-clear" onClick={() => { setQ(''); setStatus(''); setOwner(''); setCategory(''); setPage(1); }}>Filtreleri Temizle</button>
            </div>
            <div className="blocker-filters">
              <label className="blocker-search"><Search size={17} /><input value={q} onChange={(event) => { setQ(event.target.value); setPage(1); }} placeholder="Müşteri, ürün, engel veya sorumlu ara" /></label>
            ...
            <div className="blocker-status-tabs">
              {statusTabs.map((item) => (
                <button key={item.value || 'all'} type="button" className={status === item.value ? 'active' : ''} onClick={() => { setStatus(item.value); setPage(1); }}>
          ...
          <section className="blocker-list-card">
            <div className="blocker-list-head">
              <div><span>Aktif Forecast Projeleri</span><strong>{numberFormat(total)} kayıt · Sayfa {page}/{totalPages}</strong></div>
              <div className="blocker-pagination"><button type="button" disabled={page <= 1 || loading} onClick={() => setPage((value) => Math.max(1, value - 1))}>Önceki</button><button type="button" disabled={page >= totalPages || loading} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Sonraki</button></div>
            </div>

            {loading ? <div className="blocker-empty"><RefreshCw size={25} className="spin" /><strong>Projeler yükleniyor</strong></div> : rows.length === 0 ? <div className="blocker-empty"><CheckCircle2 size={30} /><strong>Bu filtrede proje bulunamadı</strong><span>Filtreleri temizleyerek tüm aktif projeleri görüntüleyebilirsiniz.</span></div> : (
              <>
                <div className="blocker-table-wrap">
                  <table className="blocker-table">
                    <thead><tr><th>Proje</th>{isAdmin ? <th>Account</th> : null}<th>Mevcut Plan</th><th>Engel / Sorumlu</th><th>Gecikme Etkisi</th><th>Durum</th><th aria-label="Aksiyon" /></tr></thead>
                    <tbody>
                      {rows.map((row) => (
                        <tr key={row.forecast_id} className={row.effective_status === 'pending' || row.effective_status === 'overdue' ? `row-${row.effective_status}` : ''}>
                          <td><div className="project-cell"><strong>{row.musteri}</strong><span>{row.product_code_snapshot ? `${row.product_code_snapshot} · ` : ''}{row.product_name_snapshot}</span></div></td>
                          ...
                          <td><div className="plan-cell"><strong>{row.forecast_period_label}</strong><span>{numberFormat(row.quantity)} adet</span></div></td>
                          ...
                          <td><div className={row.impact_type === 'month_shift' ? 'impact-cell has-impact' : 'impact-cell'}><strong>{row.impact_type === 'month_shift' ? `${numberFormat(row.shifted_quantity)} adet` : 'Etki yok'}</strong><span>{row.impact_type === 'month_shift' ? `${row.forecast_period_label} → ${row.shift_period_label}` : 'Mevcut satış planı korunuyor'}</span></div></td>
                ...
                <div className="blocker-mobile-list">
                  {rows.map((row) => (
                    <article key={row.forecast_id} className={`blocker-mobile-card state-${row.effective_status}`}>
                      <div className="mobile-card-top"><div><strong>{row.musteri}</strong><span>{row.product_code_snapshot ? `${row.product_code_snapshot} · ` : ''}{row.product_name_snapshot}</span></div><span className={`blocker-status tone-${statusTone(row.effective_status)}`}>{statusLabel(row.effective_status)}</span></div>
                      <div className="mobile-plan"><span><b>{row.forecast_period_label}</b> · {numberFormat(row.quantity)} adet</span>{isAdmin ? <span>Account: <b>{row.sorumlu || 'Atanmamış'}</b></span> : null}</div>
                      {row.effective_status === 'pending' ? <div className="mobile-pending"><CircleAlert size={18} /><span>Bu proje için değerlendirme bekleniyor.</span></div> : row.has_blocker ? <div className="mobile-detail-grid"><div><small>Engel</small><strong>{categoryLabel(row.blocker_category)}</strong><span>{row.blocker_description}</span></div><div><small>Kim / Tarih</small><strong>{row.resolution_owner_name}</strong><span>{formatDate(row.resolution_due_date)}</span></div><div><small>Gecikme Etkisi</small><strong>{row.impact_type === 'month_shift' ? `${numberFormat(row.shifted_quantity)} adet` : 'Etki yok'}</strong><span>{riskText(row)}</span></div></div> : <div className="mobile-no-obstacle"><CheckCircle2 size={18} /> Satışın önünde engel bulunmuyor.</div>}
      ...
      {view === 'team' && isAdmin ? (
        <section className="blocker-report-card">
          <div className="report-card-head"><div><span>Ekip Takibi</span><h2>Kullanıcı Tamamlama Durumu</h2><p>Kimlerin aktif projelerini cevapladığını ve kimlerin aksiyon beklediğini tek bakışta görün.</p></div><Users size={24} /></div>
          ...
          <div className="blocker-table-wrap team-table-wrap"><table className="blocker-table team-table"><thead><tr><th>Kullanıcı</th><th>Aktif Proje</th><th>Cevaplanan</th><th>Cevap Bekleyen</th><th>Açık Engel</th><th>Tarihi Geçen</th><th>Tamamlama</th></tr></thead><tbody>{completion.map((item) => <tr key={item.owner}><td><div className="owner-cell"><span className="owner-avatar">{item.owner.charAt(0).toUpperCase()}</span><strong>{item.owner}</strong></div></td><td>{numberFormat(item.activeProjects)}</td><td>{numberFormat(item.answered)}</td><td><button type="button" className={item.pending ? 'metric-link danger' : 'metric-link'} onClick={() => { setView('projects'); setOwner(item.owner); setStatus('pending'); setPage(1); }}>{numberFormat(item.pending)}</button></td><td>{numberFormat(item.open)}</td><td>{numberFormat(item.overdue)}</td><td><div className="completion-cell"><div><i style={{ width: `${item.completionRate}%` }} /></div><strong>%{item.completionRate}</strong></div></td></tr>)}</tbody></table></div>
          <div className="team-mobile-list">{completion.map((item) => <article key={item.owner}><div className="team-mobile-top"><span className="owner-avatar">{item.owner.charAt(0).toUpperCase()}</span><div><strong>{item.owner}</strong><span>{item.activeProjects} aktif proje</span></div><b>%{item.completionRate}</b></div><div className="completion-cell"><div><i style={{ width: `${item.completionRate}%` }} /></div></div><div className="team-mobile-metrics"><span>Cevaplanan <b>{item.answered}</b></span><button type="button" onClick={() => { setView('projects'); setOwner(item.owner); setStatus('pending'); setPage(1); }}>Bekleyen <b>{item.pending}</b></button><span>Geciken <b>{item.overdue}</b></span></div></article>)}</div>
        </section>
      ) : null}

      {view === 'budget' && isAdmin ? (
        <section className="blocker-report-card">
          <div className="report-card-head"><div><span>Bütçe Görünümü</span><h2>Aylık Satış Kayması</h2><p>Açık engeller çözülmezse satış adetlerinin hangi aydan hangi aya kayacağını gösterir. Forecast verisinin kendisi değiştirilmez.</p></div><BarChart3 size={24} /></div>
        ...
        <div className="blocker-drawer-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeForm(); }}>
          <aside className="blocker-drawer" role="dialog" aria-modal="true" aria-label="Engel ve Etki Bildirimi">
            <header className="drawer-head">
              <div><span>Engel ve Etki Bildirimi</span><h2>{selected.musteri}</h2><p>{selected.product_code_snapshot ? `${selected.product_code_snapshot} · ` : ''}{selected.product_name_snapshot}</p></div>
              <button type="button" onClick={closeForm} aria-label="Kapat"><X size={20} /></button>
            </header>
            <div className="drawer-project-summary"><div><span>Mevcut satış planı</span><strong>{selected.forecast_period_label}</strong></div><div><span>Forecast adedi</span><strong>{numberFormat(selected.quantity)} adet</strong></div>{isAdmin ? <div><span>Account</span><strong>{selected.sorumlu || 'Atanmamış'}</strong></div> : null}</div>

            <div className="drawer-body">
              <section className="question-block">
                <div className="question-title"><b>1</b><div><h3>Satışın önünde engel var mı?</h3><p>Projeyi durduran veya satış tarihini riske atan bir konu bulunuyor mu?</p></div></div>
                <div className="choice-grid two">
                  <button type="button" className={form.hasBlocker === 'no' ? 'choice active success' : 'choice'} onClick={() => setForm((value) => ({ ...value, hasBlocker: 'no', blockerCategory: '', blockerDescription: '', resolutionOwnerType: '', resolutionOwnerName: '', resolutionDueDate: '', impactType: 'none', shiftMonth: '', shiftedQuantity: '', workflowStatus: 'open' }))}><CheckCircle2 size={20} /><span><strong>Engel yok</strong><small>Satış planı normal ilerliyor</small></span></button>
                  <button type="button" className={form.hasBlocker === 'yes' ? 'choice active danger' : 'choice'} onClick={() => setForm((value) => ({ ...value, hasBlocker: 'yes' }))}><AlertTriangle size={20} /><span><strong>Engel var</strong><small>Takip edilmesi gereken konu var</small></span></button>
                </div>
                {form.hasBlocker === 'yes' ? <div className="question-fields"><label><span>Engel türü *</span><select value={form.blockerCategory} onChange={(event) => setForm((value) => ({ ...value, blockerCategory: event.target.value }))}><option value="">Seçiniz</option>{BLOCKER_CATEGORIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label><span>Engeli net olarak açıklayın *</span><textarea rows={4} value={form.blockerDescription} onChange={(event) => setForm((value) => ({ ...value, blockerDescription: event.target.value }))} placeholder="Örnek: Müşterinin pilot mağaza listesini paylaşması bekleniyor." /><small>{form.blockerDescription.length}/500</small></label></div> : null}
              </section>

              {form.hasBlocker === 'yes' ? (
                <section className="question-block">
                  <div className="question-title"><b>2</b><div><h3>Kim, hangi tarihte çözecek?</h3><p>Sorumlu tarafı ve gerçekçi hedef tarihi belirtin.</p></div></div>
                  <div className="question-fields grid-two"><label><span>Sorumlu taraf *</span><select value={form.resolutionOwnerType} onChange={(event) => setForm((value) => ({ ...value, resolutionOwnerType: event.target.value }))}><option value="">Seçiniz</option>{RESOLUTION_OWNER_TYPES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label><span>Planlanan çözüm tarihi *</span><input type="date" value={form.resolutionDueDate} onChange={(event) => setForm((value) => ({ ...value, resolutionDueDate: event.target.value }))} /></label><label className="full"><span>Çözüm sorumlusu *</span><input value={form.resolutionOwnerName} onChange={(event) => setForm((value) => ({ ...value, resolutionOwnerName: event.target.value }))} placeholder="Kişi, müşteri ekibi, banka veya iş ortağı" /></label><label className="full"><span>Takip durumu</span><select value={form.workflowStatus} onChange={(event) => setForm((value) => ({ ...value, workflowStatus: event.target.value as FormState['workflowStatus'] }))}><option value="open">Çözüm bekleniyor</option><option value="in_progress">Çözüm çalışması başladı</option></select></label></div>
                </section>
              ) : null}

              {form.hasBlocker === 'yes' ? (
                <section className="question-block">
                  <div className="question-title"><b>3</b><div><h3>Gecikirse satış nasıl etkilenecek?</h3><p>Satış planında ay veya adet kayması öngörülüyor mu?</p></div></div>
                  <div className="choice-grid two"><button type="button" className={form.impactType === 'none' ? 'choice active success' : 'choice'} onClick={() => setForm((value) => ({ ...value, impactType: 'none', shiftMonth: '', shiftedQuantity: '' }))}><CheckCircle2 size={20} /><span><strong>Kayma beklenmiyor</strong><small>Mevcut ay ve adet korunacak</small></span></button><button type="button" className={form.impactType === 'month_shift' ? 'choice active warning' : 'choice'} onClick={() => setForm((value) => ({ ...value, impactType: 'month_shift' }))}><CalendarClock size={20} /><span><strong>Başka aya kayacak</strong><small>Bütçe etkisi oluşacak</small></span></button></div>
                  {form.impactType === 'month_shift' ? <div className="question-fields grid-three"><label><span>Kayacağı ay *</span><select value={form.shiftMonth} onChange={(event) => setForm((value) => ({ ...value, shiftMonth: event.target.value }))}><option value="">Ay seçiniz</option>{FORECAST_MONTHS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label><span>Kayacağı yıl *</span><select value={form.shiftYear} onChange={(event) => setForm((value) => ({ ...value, shiftYear: event.target.value }))}>{YEARS.map((year) => <option key={year} value={year}>{year}</option>)}</select></label><label><span>Kayacak adet *</span><input type="number" min={1} max={selected.quantity} value={form.shiftedQuantity} onChange={(event) => setForm((value) => ({ ...value, shiftedQuantity: event.target.value }))} placeholder={`En fazla ${selected.quantity}`} /></label><div className="impact-preview"><span>Olası satış kayması</span><strong>{selected.forecast_period_label} <ArrowRight size={16} /> {form.shiftMonth ? periodLabel(form.shiftYear, form.shiftMonth) : 'Yeni dönem'}</strong><small>{form.shiftedQuantity ? `${numberFormat(form.shiftedQuantity)} adet risk altında` : 'Kayacak adedi girin'}</small></div></div> : null}
                </section>
              ) : null}

              {isAdmin && form.hasBlocker ? (
                <section className="manager-review-block"><div className="question-title"><b><UserRoundCheck size={18} /></b><div><h3>Yönetim incelemesi</h3><p>Kullanıcı cevabına yönetim notu ekleyin ve incelendi olarak işaretleyin.</p></div></div><label><span>Yönetim notu</span><textarea rows={3} value={form.managerNote} onChange={(event) => setForm((value) => ({ ...value, managerNote: event.target.value }))} placeholder="Karar, takip notu veya yönlendirme..." /></label><label className="review-check"><input type="checkbox" checked={form.reviewed} onChange={(event) => setForm((value) => ({ ...value, reviewed: event.target.checked }))} /><span><strong>Yönetim tarafından incelendi</strong><small>Proje listesinde incelendi işareti gösterilir.</small></span></label></section>
              ) : null}
            </div>
