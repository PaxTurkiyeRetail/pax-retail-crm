  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch('/api/requests/create', {
        ...
        body: JSON.stringify({ title, description, category_id: categoryId || null, priority, assignee_id: assigneeId || null, due_at: dueAt || null }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.message || 'Kayıt başarısız');
      if (onCreated) { onCreated(); } else { window.location.href = `/requests/${json.id}`; }
