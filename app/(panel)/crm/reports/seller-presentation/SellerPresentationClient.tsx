...
export default function SellerPresentationPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/seller-presentation?${params.toString()}`, { cache: 'no-store' });
  ...
  const downloadPptx = async () => {
    ...
    try {
      ...
      if (owner) params.set('owner', owner);
      if (segment) params.set('segment', segment);
      if (!owner) { const msg = 'Satışçı seçmeden PPTX indirilemez.'; setMessage(msg); appToast.error('Satışçı seçimi gerekli', msg); return; }
      const res = await fetch(`/api/reports/seller-presentation/pptx?${params.toString()}`, { cache: 'no-store' });
      if (!res.ok) {
        const json = await res.json().catch(() => ({}));
      ...
      link.href = href;
      link.download = `satisci-sunumu-${owner || 'account'}-${from}-${to}.pptx`;
      link.click();
  ...
  const sellerSelected = Boolean(owner);
  const canDownloadPptx = sellerSelected && Boolean(payload.customers.length) && !loading && !downloadingPptx;
  const pptxDisabledReason = !sellerSelected
    ? 'PPTX indirmek için önce Satışçı / Account seçmelisin.'
    : !payload.customers.length
      ? 'Seçili satışçı için müşteri verisi bulunamadı.'
      : '';

  return (
    <main className="wmp-page pax-page-container">
      <section className="wmp-hero">
        <span className="wmp-eyebrow">Rapor Merkezi · Satışçı Sunumu</span>
        <h1>{payload.narrative.title || 'Satışçı Sunumu'}</h1>
        <p>
          Seçilen satışçıya/accounta bağlı müşterileri ve sadece o accountun aktivitelerini PPTX şablonuna basan özel sunum ekranı. Toplu indirme yoktur; her satışçı ayrı filtrelenip ayrı indirilir.
        </p>
        <div className="wmp-actions">
          <button onClick={() => void load()} disabled={loading}>{loading ? 'Yükleniyor...' : '↻ Yenile'}</button>
          <button
            className="secondary seller-pptx-button"
            onClick={() => void downloadPptx()}
            disabled={!canDownloadPptx}
            title={pptxDisabledReason || 'Seçili satışçı için PPTX indir'}
          >
            {downloadingPptx ? 'PPTX hazırlanıyor...' : sellerSelected ? 'PPTX İndir' : 'Önce Satışçı Seç'}
          </button>
      ...
      <section className="wmp-filter-card">
        <div className="wmp-filter-grid">
          ...
          <label className={!sellerSelected ? 'wmp-required-filter missing' : 'wmp-required-filter'}>
            <span>Satışçı / Account <em>Zorunlu</em></span>
            <select value={owner} onChange={(e) => setOwner(e.target.value)} aria-invalid={!sellerSelected}>
        ...
        {!sellerSelected ? (
          <div className="wmp-selection-warning">
            <strong>Satışçı seçimi zorunlu.</strong> PPTX indirmek için önce yukarıdan bir Satışçı / Account seçmelisin. Seçim yapılana kadar indirme butonu kapalıdır.
          </div>
        ) : null}
