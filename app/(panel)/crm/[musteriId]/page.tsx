...
import { ENTEGRASYON_OPTIONS, HAVUZ_ACCOUNT_NAME, SATIS_OLASILIGI_OPTIONS } from '@/lib/crm';
...
type AllowedUser = { email?: string; full_name?: string | null };
type FilterOptions = { sectorOptions?: string[] };
...
export default function CustomerDetailPage() {
  ...
  const [sectors, setSectors] = useState<string[]>(SECTOR_PRESET_OPTIONS);

  const load = async () => {
    setMsg(null);
    const [detailRes, usersRes, optionsRes] = await Promise.all([
      fetch(`/api/crm/detail?musteriId=${musteriId}`, { cache: 'no-store' }),
      fetch('/api/allowed-users-lite', { cache: 'no-store' }),
      fetch('/api/crm/options', { cache: 'no-store' }),
    ]);

    const payload = await detailRes.json().catch(() => ({}));
    if (!detailRes.ok) return setMsg(payload?.message || 'Detay yüklenemedi');
    ...
    if (usersRes.ok) {
      ...
      setOwners(uniqueOptions([...(usersJson.users ?? []).map((x: AllowedUser) => x.full_name), HAVUZ_ACCOUNT_NAME, musteri.sorumlu]));
    } else {
      setOwners(uniqueOptions([HAVUZ_ACCOUNT_NAME, musteri.sorumlu]));
    }

    if (optionsRes.ok) {
      const optionsJson = await optionsRes.json().catch(() => ({})) as FilterOptions;
      setSectors(uniqueOptions([...SECTOR_PRESET_OPTIONS, ...(optionsJson.sectorOptions ?? []), musteri.sektor]));
    } else {
      setSectors(uniqueOptions([...SECTOR_PRESET_OPTIONS, musteri.sektor]));
    }
  };
  ...
  const setDetailField = (key: keyof CustomerDetail, value: string | null) => setDetail((prev) => prev ? { ...prev, [key]: value } : prev);
  ...
  const saveAll = async () => {
    ...
    setMsg(null);
    try {
      const customerRes = await fetch('/api/crm/update', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          musteriId: detail.id,
          musteri: detail.musteri,
          sektor: detail.sektor,
          sorumlu: detail.sorumlu,
          entegrasyon_tipi: detail.entegrasyon_tipi,
          satis_olasiligi: detail.satis_olasiligi,
        })
      });
      ...
      if (hasKunyeChanges) {
        const kuyeRes = await fetch('/api/kunye', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
            musteriId: detail.id,
            ...kunye,
            pos_mulkiyet_bankalari: ['Banka', 'Bankada'].includes(String(kunye.pos_mulkiyet ?? '')) ? kunye.pos_mulkiyet_bankalari : null,
          })
        });
      ...
      setMsg(customerJson?.message || 'Müşteri ve künye bilgileri kaydedildi.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <main className="page">
      <style jsx>{`
        ...
        .input, .select, .textarea { min-height: 40px; border-radius: 12px; border: 1px solid #d5dee8; padding: 0 12px; font-size: 13px; width: 100%; background: #fff; }
        ...
        @media (max-width: 920px) {
          .grid, .stats, .two, .status-strip { grid-template-columns: 1fr; }
          .actions { width:100%; }
          .actions > :global(button) { flex:1 1 auto; }
        }
      `}</style>

      <section className="hero">
        <div>
          <h1 className="title">{detail?.musteri ?? 'Müşteri Detayı'}</h1>
          <div className="sub">Liste ekranındaki düzenleme aksiyonları bu sayfaya taşındı. Müşteri temel bilgileri ve künye artık tek noktadan yönetiliyor.</div>
        </div>
        <div className="actions">
          <button className="secondary" onClick={() => history.back()}>Geri</button>
          <button className="primary" onClick={saveAll} disabled={saving}>{saving ? 'Kaydediliyor...' : 'Kaydet'}</button>
        </div>
      </section>

      <section className="stats">
        <div className="stat"><div className="stat-label">Müşteri Adı</div><div className="stat-value">{detail?.musteri ?? '-'}</div></div>
        <div className="stat"><div className="stat-label">Sektör</div><div className="stat-value">{detail?.sektor ?? '-'}</div></div>
        <div className="stat"><div className="stat-label">Sorumlusu</div><div className="stat-value">{detail?.sorumlu ?? '-'}</div></div>
      </section>

      {msg ? <div className="message">{msg}</div> : null}

      <section className="status-strip">
        <div className="stat"><div className="stat-label">Künye Durumu</div><div className="stat-value">{status.status}</div></div>
        <div className="stat"><div className="stat-label">Eksik Alan</div><div className="stat-value">{status.missing}</div></div>
        <div className="stat"><div className="stat-label">Mobil Uyum</div><div className="stat-value">Hazır</div></div>
      </section>

      <section className="surface">
        <h2 className="section-title">Temel Müşteri Bilgileri</h2>
        <div className="grid">
          <label className="field">
            <span className="label">Müşteri Adı</span>
            <input className="input" value={detail?.musteri ?? ''} onChange={(e) => setDetailField('musteri', e.target.value)} />
          </label>
          <label className="field">
            <span className="label">Sektör</span>
            <select className="select" value={detail?.sektor ?? ''} onChange={(e) => setDetailField('sektor', e.target.value || null)}>
          ...
          <label className="field">
            ...
            <select className="select" value={detail?.sorumlu ?? ''} onChange={(e) => setDetailField('sorumlu', e.target.value || null)}>
              <option value="">Seçiniz</option>
              {owners.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
            </select>
          </label>
          <label className="field">
            <span className="label">Entegrasyon Yapısı</span>
            <select className="select" value={detail?.entegrasyon_tipi ?? ''} onChange={(e) => setDetailField('entegrasyon_tipi', e.target.value || null)}>
              {ENTEGRASYON_OPTIONS.map((opt) => <option key={opt || 'bos'} value={opt}>{opt || 'Seçilmedi'}</option>)}
            </select>
          </label>
          <label className="field">
            <span className="label">Satış Olasılığı</span>
            <select className="select" value={detail?.satis_olasiligi ?? ''} onChange={(e) => setDetailField('satis_olasiligi', e.target.value || null)}>
              {SATIS_OLASILIGI_OPTIONS.map((opt) => <option key={opt || 'bos'} value={opt}>{opt || 'Seçilmedi'}</option>)}
            </select>
          </label>
        </div>
      </section>

      <section className="surface"><h2 className="section-title">1) Firma Bilgisi</h2><div className="grid"><label className="field"><span className="label">Müşteri Adı</span><input className="input" value={detail?.musteri ?? ''} onChange={(e) => setDetailField('musteri', e.target.value)} /></label><label className="field"><span className="label">Mağaza Sayısı</span><input className="input" value={kunye.magaza_sayisi ?? ''} onChange={(e) => setField('magaza_sayisi', e.target.value)} /></label><label className="field"><span className="label">Franchise Sayısı</span><input className="input" value={kunye.franchise_sayisi ?? ''} onChange={(e) => setField('franchise_sayisi', e.target.value)} /></label></div></section>
