...
export default function MusteriPage() {
  ...
  const musteriId = params.musteriId;
  ...
  const load = async () => {
    ...
    const [timelineRes, detailRes, usersRes] = await Promise.all([
      fetch(`/api/crm/timeline?musteriId=${musteriId}`),
      fetch(`/api/crm/detail?musteriId=${musteriId}`),
    ...
    if (!timelineRes.ok) {
      setMsg(
        timelineJson?.error === "FORBIDDEN"
          ? "Bu müşteri kaydını görüntüleme yetkin yok."
          : (timelineJson?.error ?? "Hata")
      );
      setEvents([]);
    } else {
  ...
  const addEvent = async () => {
    ...
    const res = await fetch("/api/pipeline/event", {
      ...
      body: JSON.stringify({ musteriId, fazNo, eventType: "note", notlar }),
    });
  ...
  const saveKunye = async () => {
    ...
    try {
      const res = await fetch("/api/kunye", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ musteriId, ...kunye }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
