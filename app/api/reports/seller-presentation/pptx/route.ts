...
export async function GET(request: Request) {
  try {
    ...
    if (!owner) return NextResponse.json({ message: 'Satışçı seçmeden PPTX indirilemez.' }, { status: 400 });
    const { buffer } = await generateWeeklyManagementPresentationPptx(admin, { from, to, owner, segment, sellerMode: true });
    return new NextResponse(new Uint8Array(buffer), {
      status: 200,
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
        'Content-Disposition': contentDispositionAttachment(`satisci-sunumu-${owner || 'account'}-${from || 'auto'}-${to || 'auto'}.pptx`),
