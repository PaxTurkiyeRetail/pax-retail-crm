...
export async function POST(request: Request) {
  try {
    ...
    if (!customerId) return NextResponse.json({ message: 'Müşteri kaydı zorunlu.' }, { status: 400 });
    ...
    if (hasBlocker) {
      ...
      if (!BLOCKER_CATEGORIES.some((item) => item.value === blockerCategory)) {
        return NextResponse.json({ message: 'Engel kategorisi seçilmelidir.' }, { status: 400 });
      }
      if (blockerDescription.length < 5) {
        return NextResponse.json({ message: 'Engel açıklamasını daha net yazın.' }, { status: 400 });
      }
      if (!RESOLUTION_OWNER_TYPES.some((item) => item.value === ownerType)) {
        return NextResponse.json({ message: 'Çözüm sorumlusu tipi seçilmelidir.' }, { status: 400 });
      }
      if (ownerName.length < 2) {
        return NextResponse.json({ message: 'Çözüm sorumlusu yazılmalıdır.' }, { status: 400 });
      }
      if (!dueDate) {
        return NextResponse.json({ message: 'Planlanan çözüm tarihi seçilmelidir.' }, { status: 400 });
      }

      if (requestedForecastId) {
        await getCustomerForecastOrThrow(customerId, requestedForecastId);
        forecastId = requestedForecastId;
      }

      if (impactType === 'month_shift') {
        if (!forecastId) {
          return NextResponse.json({ message: 'Bütçe kayması için etkilenen Forecast seçilmelidir.' }, { status: 400 });
        }
        const forecast = await getCustomerForecastOrThrow(customerId, forecastId);
        shiftYear = Number(body.shift_year ?? 0);
        shiftMonth = Number(body.shift_month ?? 0);
        shiftedQuantity = Number(body.shifted_quantity ?? 0);

        if (
          !Number.isInteger(shiftYear)
          || shiftYear < 2024
          || shiftYear > 2100
          || !Number.isInteger(shiftMonth)
          || shiftMonth < 1
          || shiftMonth > 12
        ) {
        ...
        if (!isLaterPeriod(forecast.forecast_year, forecast.forecast_month, shiftYear, shiftMonth)) {
          return NextResponse.json({ message: 'Kayacağı dönem seçilen Forecast döneminden ileri olmalıdır.' }, { status: 400 });
        }
        if (!Number.isInteger(shiftedQuantity) || shiftedQuantity <= 0) {
          return NextResponse.json({ message: 'Kayacak adet sıfırdan büyük olmalıdır.' }, { status: 400 });
        }
        if (shiftedQuantity > Number(forecast.quantity)) {
          return NextResponse.json({ message: `Kayacak adet seçilen Forecast'taki ${forecast.quantity} adedi aşamaz.` }, { status: 400 });
        }
      }
    }
    ...
    try {
      const result = await db.query(
        `
          ...
          returning id::text, customer_id::text, forecast_id::text, workflow_status, updated_at::text
        `,
        [
          customerId,
          forecastId,
      ...
      return NextResponse.json({ ok: true, customer: customer.musteri, row: result.rows[0] });
    } catch (error) {
