...
async function customerForAccess(client: Pick<PoolClient, 'query'>, customerId: string) {
  const row = (await client.query('select id,owner_user_id,sorumlu,customer_type,is_ortagi_tipi from public.musteriler where id=$1', [customerId])).rows[0];
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ rows: rows.rows, can_edit: canEdit });
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const before = (await client.query('select role_key,subtype,is_active,version from public.organization_roles where customer_id=$1 order by role_key for update', [input.customer_id])).rows;
    ...
    await client.query('update public.musteriler set customer_type=$2,is_ortagi_tipi=$3,updated_at=now() where id=$1',
      [input.customer_id, legacyType, input.business_partner ? (input.partner_subtype || null) : null]);
    ...
    await recordAuditEvent({ actorId: user.id, actorEmail: user.email, action: 'customer.relationships.updated', resourceType: 'customer', resourceId: input.customer_id, before, after }, client);
    await client.query('commit');
    return NextResponse.json({ rows: after });
